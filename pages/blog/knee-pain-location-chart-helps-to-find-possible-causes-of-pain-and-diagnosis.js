...
const Slug = ({ items }) => {
  return items ? (
    <>
      <Head>
        <title>{items && items?.seoTitle}</title>
        <link
          rel="canonical"
        ...
        />
        <meta property="og:title" content={items && items?.seoTitle} />
...
export const getServerSideProps = async () => {
  const posts = await axios.get(
    SSR_ENDPOINT +
      ...
      "knee-pain" +
      "/" +
      "knee-pain-location-chart-helps-to-find-possible-causes-of-pain-and-diagnosis" +
      "/"
  );
  console.log(posts);
