...
export const News = ({ item, totalPage }) => {
  ...
  const loadMoreData = async (page) => {
    const posts = await axios.get(
      `${CSR_ENDPOINT}news/getAll/pagination?page=${page}&limit=21`
    );
  ...
  return (
    <>
      <Head>
        <title>Latest Pain Management News | Premier Pain Centers</title>
        <meta content="text/html; charset=utf-8" httpEquiv="Content-Type" />
        <meta content="initial-scale=1" name="viewport" />
        <meta httpEquiv="X-UA-Compatible" content="IE=edge,chrome=1" />
        <meta
          name="description"
          content="Check out our collection of News that cover topics such as chronic pain, alternative therapies, and the latest medical treatments."
        />
                ...
                <div className="card-container-blog">
                  {items?.map((data, index) => (
                    <div className="card-blog" key={index}>
                      ...
                      <Link className="button" href={`/news${data.slug}`}>
                        Read More
                      </Link>
                    </div>
                  ))}
...
export const getServerSideProps = async () => {
  const posts = await axios.get(
    SSR_ENDPOINT + "news/getAll/pagination?page=1&limit=21"
  );
  const data = await posts.data.data;
